...
%DIF DEL paper_prev_draft.tex   Sun Apr 28 00:02:01 2024
%DIF ADD paper.tex              Mon Apr 29 14:05:04 2024
...
\begin{frontmatter}
  ...
  \title{\DIFdelbegin \DIFdel{Study of mixing behaviour }\DIFdelend \DIFaddbegin {\DIFadd{DEM}}\DIFadd{-}{\DIFadd{SPH}} \DIFadd{study }\DIFaddend of \DIFdelbegin \DIFdel{spherical }\DIFdelend particles \DIFaddbegin \DIFadd{dispersion }\DIFaddend in fluid\DIFdelbegin \DIFdel{using stirrer}\DIFdelend }
...
industry \cite{wang2023developments}, \DIFdelbegin \DIFdel{The }\DIFdelend \DIFaddbegin \DIFadd{the }\DIFaddend transport of bodies in internal
...
few areas to mention.  \DIFdelbegin \DIFdel{Particularly, processes involving }\DIFdelend \DIFaddbegin \DIFadd{In many processes }\DIFaddend the mixing of powder with fluid using
a stirrer are commonplace \cite{li2022study}.  \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\Cref{fig:gloabl_body_frame_rb}
}\hspace{0pt}%DIFAUXCMD
illustrates a sample particle dispersion problem studied in the current work.
}\DIFaddend While experimental investigation of such processes is often impractical due to
their highly nonlinear nature, numerical methods offer a viable alternative.
\DIFdelbegin \DIFdel{These
}\DIFdelend \DIFaddbegin \DIFadd{While these }\DIFaddend systems are studied \DIFdelbegin \DIFdel{as }\DIFdelend \DIFaddbegin \DIFadd{with numerical methods, they are a }\DIFaddend part of
two-way coupling models. \DIFdelbegin \DIFdel{The two-way coupling
phenomena are nonlinear, and an analytical study is not feasible. A numerical study is preferable while handling such a
phenomenon. A }\DIFdelend \DIFaddbegin \DIFadd{In numerical method approach, one can employ either a
}\DIFaddend mesh-based or meshless technique \DIFdelbegin \DIFdel{can be utilized to study rigid fluid coupling (RFC) numerically.
\mbox{%DIFAUXCMD
\Cref{fig:gloabl_body_frame_rb} }\hspace{0pt}%DIFAUXCMD
illustrates a sample particle dispersion
problem studied in the current work.
}\DIFdelend \DIFaddbegin \DIFadd{to study the current phenomena.
}
...
mesh-based \DIFdelbegin \DIFdel{CFD-DEM }\DIFdelend modeling, lattice Boltzmann method (LBM)
...
\DIFaddend While handling the interaction between the fluid and solid particles, the
interaction can be categorized into a resolved and unresolved coupling. In
unresolved coupling, simplified models compute forces acting on solid
particles within fluid flow, \DIFdelbegin \DIFdel{while fully resolved coupling computes
forces comprehensively}\DIFdelend \DIFaddbegin \todoin{while fully resolved coupling computes
  forces comprehensively}\DIFaddend . Both strategies are applicable in both meshless
\cite{cleary2015prediction, trujillo2020smooth} and mesh-based
\cite{van2008numerical, ma2022review} CFD-DEM techniques.  \DIFdelbegin \DIFdel{Research }\DIFdelend \DIFaddbegin \DIFadd{In addition to the
study mono-sized particle behaviour in a fluid \mbox{%DIFAUXCMD
\cite{trujillo2020smooth}}\hspace{0pt}%DIFAUXCMD
,
research }\DIFaddend such as \cite{brosh2014accelerating} and \cite{peng2021fully} explore
wide particle size distributions with CFD-DEM and SPH-DEM respectively.
Regarding non-Newtonian fluid \DIFdelbegin \DIFdel{modeling }\DIFdelend \DIFaddbegin \DIFadd{modelling }\DIFaddend in mesh-based schemes, works like
...
\cite{canelas2016sph} with fully resolved coupling approach \DIFdelbegin \DIFdel{has been
solved }\DIFdelend \DIFaddbegin \DIFadd{were analysed
}\DIFaddend using SPH-DEM\DIFdelbegin \DIFdel{framework}\DIFdelend .  Handling particles of arbitrary shape in fluid flow is
...
WCSPH \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{peng2021fully, cleary2015}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{peng2021fully, cleary2015prediction}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend , and ISPH
...
structures discretized as vertex edges is \DIFdelbegin \DIFdel{proposed by
\mbox{%DIFAUXCMD
\cite{park2023new}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{also proposed by
\mbox{%DIFAUXCMD
\citet{park2023new}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . Particle settling with variable sizes is studied by
...
discussed in papers by \citet{asai2021fluid}, \DIFdelbegin \DIFdel{citet}%DIFDELCMD < {%%%
\DIFdel{peng2021fully}%DIFDELCMD < }%%%
\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citet{peng2021fully}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend ,
\citet{canelas2016sph}.



\DIFaddbegin \todoin{Current research gap}\DIFadd{. }\DIFaddend In the current work we cosider the particle
dispersion within a large-scale under stirrer mixing phenomenon. We employ
Smoothed Particle Hydrodynamics (SPH) to model fluid dynamics and the Discrete
Element Method (DEM) to handle the dynamics and contact interactions among
spherical particles. The interaction between solids and fluids is \DIFdelbegin \DIFdel{managed }\DIFdelend \DIFaddbegin \DIFadd{modelled
}\DIFaddend using a fixed ghost particle approach, ensuring a fully resolved
simulation. We validate the fluid solver independently using Poiseuille flow
and the DEM solver separately using fundamental benchmarks such as
particle-particle and particle-wall impacts. Subsequently, the fully resolved
coupled SPH-DEM solver is validated using three benchmarks: a single particle
entering, two particles settling, and a forced wedge entry into a steady
hydrostatic tank. Upon confirming the fidelity of the solver, we proceed with
analyzing the mixing behavior of various spherical particles influenced by a
stirrer in a \DIFdelbegin \DIFdel{water }\DIFdelend tank. We investigate scenarios involving particles of different
densities, stirrer velocities, and variable sizes as part of our case
study. The development work is conducted using
PySPH\cite{ramachandran2021pysph}, an open-source code available at
\url{https://github.com/pypr/pysph}.  \DIFaddbegin \DIFadd{We modify the current codebase to
...
research. }\DIFaddend To ensure reproducibility, we utilize the automan package
...
fluid. \DIFdelbegin \DIFdel{Smoothed particle hydrodynamics is initially developed by Lucy
\mbox{%DIFAUXCMD
\cite{lucy1977numerical} }\hspace{0pt}%DIFAUXCMD
and Gingold and Monaghan \mbox{%DIFAUXCMD
\cite{gingold1977smoothed}
}\hspace{0pt}%DIFAUXCMD
to model astrophysical problems. }\DIFdelend The continuum in SPH is modeled using particles, which has physical
properties such as mass, velocity, and \DIFdelbegin \DIFdel{the
}\DIFdelend \DIFaddbegin \DIFadd{these }\DIFaddend particles interact based on the
governing equations using a Guassian-like kernel
\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{SPH_papers}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{monaghan-review:2005,morris1997modeling}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend .
...
\DIFdelbegin \DIFdel{Equations governing the fluid motion are, }\DIFdelend \DIFaddbegin \DIFadd{The fluid motion is governed by the }\DIFaddend conservation of mass \DIFdelbegin \begin{displaymath}
  \DIFdel{%DIFDELCMD < \label{eq:ce}%%%
  \frac{d \rho}{d t} = - \rho \; \frac{\partial u_i}{\partial x_i},
}\end{displaymath}%DIFAUXCMD
\DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{and the }\DIFaddend conservation
of linear momentum, \DIFaddbegin \DIFadd{which can be expressed by the following equations:
}\begin{equation}
  \DIFadd{\label{eq:ce}
  \frac{d \rho}{d t} = - \rho \; \frac{\partial u_i}{\partial x_i},
}\end{equation}
...
equation~\cref{eq:sph-discretization-continuity} \DIFdelbegin \DIFdel{and the momentum equation
~\mbox{%DIFAUXCMD
\cref{eq:sph-momentum-fluid} }\hspace{0pt}%DIFAUXCMD
respectively are}\DIFdelend \DIFaddbegin \DIFadd{is given as}\DIFaddend ,
\begin{equation}
...
%
Similarly, the discretized momentum equation \DIFdelbegin \DIFdel{for fluids }\DIFdelend is written as,
\begin{multline}
...
fluid and \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{morris1997modeling} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \todoin{\cite{morris1997modeling}} \DIFaddend formulation is used to discretize the
viscosity term.
...
The ghost particle approach \DIFdelbegin \DIFdel{of \mbox{%DIFAUXCMD
\cite{Adami2012} }\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{proposed by \mbox{%DIFAUXCMD
\citet{Adami2012} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend is used to model the
...
\end{equation}
where $\ten{u}_b$, is the \DIFdelbegin \DIFdel{momentum }\DIFdelend velocity of the fluid \DIFaddbegin \DIFadd{particle $b$ }\DIFaddend and $W_{ab}$ is the kernel
...
We resolve the contact among the spherical particles using \DIFaddbegin \DIFadd{the }\DIFaddend discrete
element method \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{luding_dem_2008}}\hspace{0pt}%DIFAUXCMD
}\DIFaddend . In the current work we have utilized a
...
tangential component. The normal force component \DIFdelbegin \DIFdel{is utilised to make sure that the particle do not
penetrate into each other}\DIFdelend \DIFaddbegin \DIFadd{represents a repulsive force}\DIFaddend ,
...
Hetzian model \DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\citet{brilliantov1996model}}\hspace{0pt}%DIFAUXCMD
,
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{brilliantov1996model}}\hspace{0pt}%DIFAUXCMD
,
}\DIFaddend \begin{equation}
  \label{eq:contact-algorithm-normal}
  \ten{F}_a^n = k\DIFdelbegin \DIFdel{_r }\DIFdelend \DIFaddbegin \DIFadd{_n }\DIFaddend \delta_{n} \ten{n}.
\end{equation}
...
material properties of the bodies in contact\DIFdelbegin \DIFdel{\mbox{%DIFAUXCMD
\cite{golshan2023lethe}}\hspace{0pt}%DIFAUXCMD
.
}\DIFdelend \DIFaddbegin \DIFadd{, given by:
}\begin{equation}
  \DIFadd{\label{eq:kf-stiffness}
...
proportional to the tangential \DIFdelbegin \DIFdel{spring }\DIFdelend displacement, which is integrated over
the contact time as
\begin{equation}
...
We use the kick-drift-kick scheme \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{monaghan-review:2005} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend for the time integration. We move the
velocities of the fluid and the solid particles to half time step,
\begin{equation}
  \label{eq:velocity-update-stage-1}
  \ten{u}_a\DIFdelbegin \DIFdel{^{n+\frac{1}{2}} }\DIFdelend \DIFaddbegin \DIFadd{^{t+\frac{1}{2} \Delta t} }\DIFaddend = \ten{u}_a\DIFdelbegin \DIFdel{^{n} }\DIFdelend \DIFaddbegin \DIFadd{^{t} }\DIFaddend + \frac{\Delta t}{2} \bigg(\frac{d\ten{u}_{a}}{dt}\bigg)\DIFdelbegin \DIFdel{^n}\DIFdelend \DIFaddbegin \DIFadd{^t}\DIFaddend ,
\end{equation}
\DIFaddbegin \DIFadd{here, $()_a^t$ represents the properties of particle $a$ at time $t$ and
$()_a^{t+\frac{1}{2} \Delta t}$ corresponds to the halfway point between time
$t$ and $t + \Delta t$. }\DIFaddend Then the time derivative of density is calculated using the
\cref{eq:sph-discretization-continuity}\DIFdelbegin \DIFdel{using the }\DIFdelend \DIFaddbegin \DIFadd{, with }\DIFaddend velocities at half time step
\DIFdelbegin \DIFdel{. The new time step }\DIFdelend \DIFaddbegin \DIFadd{employed for the calculation.  The updated }\DIFaddend density and particle position \DIFdelbegin \DIFdel{is updated by,
}\DIFdelend \DIFaddbegin \DIFadd{for
the new time step are determined by,
}\DIFaddend \begin{equation}
  \label{eq:density-update-stage-2}
  \rho_{a}\DIFdelbegin \DIFdel{^{n+1} }\DIFdelend \DIFaddbegin \DIFadd{^{t+\Delta t} }\DIFaddend = \rho_{a}\DIFdelbegin \DIFdel{^{n} }\DIFdelend \DIFaddbegin \DIFadd{^{t} }\DIFaddend + \Delta t \; \bigg(\frac{d\rho_{a}}{dt}\bigg)\DIFdelbegin \DIFdel{^{n+\frac{1}{2}}}\DIFdelend \DIFaddbegin \DIFadd{^{t+\frac{1}{2} \Delta t}}\DIFaddend ,
\end{equation}
\begin{equation}
  \label{eq:position-update-stage-2}
  \ten{r}_{a}\DIFdelbegin \DIFdel{^{n+1} }\DIFdelend \DIFaddbegin \DIFadd{^{t+\Delta t} }\DIFaddend = \ten{r}_{a}\DIFdelbegin \DIFdel{^{n} }\DIFdelend \DIFaddbegin \DIFadd{^{t} }\DIFaddend + \Delta t \; \ten{u}_{a}\DIFdelbegin \DIFdel{^{n+1}}\DIFdelend \DIFaddbegin \DIFadd{^{t+\Delta t}}\DIFaddend .
\end{equation}
%
Finally, at new time-step particle position, the momentum velocity is updated
\begin{equation}
  \label{eq:velocity-update-stage-3}
  \ten{u}_a\DIFdelbegin \DIFdel{^{n+1} }\DIFdelend \DIFaddbegin \DIFadd{^{t+\Delta t} }\DIFaddend = \ten{u}_a\DIFdelbegin \DIFdel{^{n+\frac{1}{2}} }\DIFdelend \DIFaddbegin \DIFadd{^{t+\frac{1}{2}\Delta t} }\DIFaddend + \frac{\Delta t}{2} \bigg(\frac{d\ten{u}_{a}}{dt}\bigg)\DIFdelbegin \DIFdel{^{n+1}}\DIFdelend \DIFaddbegin \DIFadd{^{t+\Delta t}}\DIFaddend .
\end{equation}


The modeling of rigid-rigid interaction requires \DIFdelbegin \DIFdel{lower }\DIFdelend \DIFaddbegin \DIFadd{a smaller }\DIFaddend time step than the
fluid. We choose the minimum of \DIFdelbegin \DIFdel{both the }\DIFdelend \DIFaddbegin \DIFadd{these two }\DIFaddend timesteps to move the system forward
in time. For the numerical stability of fluid, the time step depends on the
\DIFdelbegin \DIFdel{CFL condition }\DIFdelend \DIFaddbegin \DIFadd{Courant–Friedrichs–Lewy (CFL) condition \mbox{%DIFAUXCMD
\cite{monaghan-review:2005} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend as,
\begin{equation}
...
step is constrained \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\cite{cundall_discrete_1979} }\hspace{0pt}%DIFAUXCMD
}\DIFaddend as,
\begin{equation}
...
\begin{figure}[!htpb]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.4\textwidth]{figures/plane_poiseuille_flow_2D/case_1/comparison.png}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.7\textwidth]{figures/plane_poiseuille_flow_2D/case_1/comparison.png}
...
\begin{figure}[!htpb]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.6\textwidth]{figures/benchmark_1_ss_colliding_elastic/fn_vs_overlap}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.8\textwidth]{figures/benchmark_1_ss_colliding_elastic/fn_vs_overlap}
...
\begin{figure}[!htpb]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.4\textwidth]{images/results_dem_2_validation_particle_wall_impact/schematic}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.6\textwidth]{images/results_dem_2_validation_particle_wall_impact/schematic}
  \DIFaddendFL \caption{Schematic of a spherical particle impacting a wall
...
magnitude. This test case is \DIFdelbegin \DIFdel{helpful }\DIFdelend \DIFaddbegin \DIFadd{useful }\DIFaddend in testing the tangential \DIFdelbegin \DIFdel{part }\DIFdelend \DIFaddbegin \DIFadd{interaction
modeling }\DIFaddend of our DEM solver.  The \DIFaddbegin \DIFadd{current problem was experimentally
investigated by \mbox{%DIFAUXCMD
\citet{kharaz2001experimental}}\hspace{0pt}%DIFAUXCMD
, and is commonly employed by
various DEM solvers to validate their codes, as seen in the work of
\mbox{%DIFAUXCMD
\cite{di2004comparison} }\hspace{0pt}%DIFAUXCMD
and \mbox{%DIFAUXCMD
\cite{golshan2023lethe}}\hspace{0pt}%DIFAUXCMD
.  The }\DIFaddend schematic of the
body and wall is shown in \cref{fig:result:dem_sw_contact_schematic}.
The \DIFaddbegin \DIFadd{dimensions and material properties of both the particle and the wall are
set based on the study of \mbox{%DIFAUXCMD
\cite{di2004comparison,kharaz2001experimental}}\hspace{0pt}%DIFAUXCMD
. The
}\DIFaddend radius of the impacting particle is \DIFdelbegin \DIFdel{$0.0025$ }\DIFdelend \DIFaddbegin \DIFadd{$2.5 \times 10^{-3}$ }\DIFaddend m and is made of aluminium
...
\begin{figure}[!htpb]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.4\textwidth]{figures/benchmark_4_sw_colliding_oblique/angle_vs_ang_vel.pdf}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.6\textwidth]{figures/benchmark_4_sw_colliding_oblique/angle_vs_ang_vel.pdf}
...
\FloatBarrier%
\subsection{RFC validation 2: \DIFdelbegin \DIFdel{Falling solid }\DIFdelend \DIFaddbegin \DIFadd{A 3D cube falling }\DIFaddend in water}
...
}\DIFdelend \cite{nan2022cfd} and other SPH-DEM solvers \cite{qiu20173d}. The cube has a
...
of the water is taken as $1000$ kg\DIFdelbegin \DIFdel{,}\DIFdelend \DIFaddbegin \DIFadd{\,}\DIFaddend m\textsuperscript{-3}, and the density of
the cube is \DIFdelbegin \DIFdel{$2120$ kg,}\DIFdelend \DIFaddbegin \DIFadd{$2,120$ kg\,}\DIFaddend m\textsuperscript{-3}. The schemtic of the initial
configuration is shown in \cref{fig:results_rfc_02_falling}. \DIFdelbegin \DIFdel{The tank and the
fluid has $150$ mm x-direction, out of the plane which is not shown in the
schematic. }\DIFdelend We show that the
...
\begin{figure}[!htpb]
  ...
  \caption{Schematic of a cube of density \DIFdelbeginFL \DIFdelFL{$2120 kg,m\textsuperscript{-3}$
    }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{$2,120 kg\,m\textsuperscript{-3}$
    }\DIFaddendFL immersed half way in a steady hydrostatic tank is allowed to settle under
    gravity.}
...
\begin{figure}[!htpb]
  \centering
  \DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.4\textwidth]{figures/wu_2014_falling_solid/y_vs_t}
%DIFDELCMD <   %%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.6\textwidth]{figures/wu_2014_falling_solid_3d/z_vs_t}
  \DIFaddendFL \caption{Vertical position variation of the center of mass of the cube with
...
\DIFaddbegin \DIFadd{In this section we study the particle dispersion in a tank with varied stirrer
velocities.
}
...
\begin{figure}[!htpb]
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{figures/dinesh_2024_mixing_with_stirrer_2d/case_1/time1}
    \subcaption{\DIFaddFL{t = $0$ ms}}\label{fig:rings:ipst-nu-0.47-1}
  \end{subfigure}
  %DIF > 
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{figures/dinesh_2024_mixing_with_stirrer_2d/case_1/time1}
    \subcaption{\DIFaddFL{t = $1.38$ ms}}\label{fig:rings:ipst-nu-0.47-2}
  \end{subfigure}

  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{figures/dinesh_2024_mixing_with_stirrer_2d/case_1/time3}
    \subcaption{\DIFaddFL{t = $5.17$ ms}}\label{fig:rings:ipst-nu-0.47-3}
  \end{subfigure}
%DIF > 
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{figures/dinesh_2024_mixing_with_stirrer_2d/case_1/time0}
    \subcaption{\DIFaddFL{t = $7.38$ ms}}\label{fig:rings:ipst-nu-0.47-4}
  \end{subfigure}
  \caption{}
...
\begin{figure}[!htpb]
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{figures/dinesh_2024_mixing_with_stirrer_2d/case_2/time1}
    \subcaption{\DIFaddFL{t = $0$ ms}}\label{fig:rings:ipst-nu-0.47-1}
  \end{subfigure}
  %DIF > 
  \begin{subfigure}{0.48\textwidth}
  ...
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{figures/dinesh_2024_mixing_with_stirrer_2d/case_2/time3}
    \subcaption{\DIFaddFL{t = $5.17$ ms}}\label{fig:rings:ipst-nu-0.47-3}
  \end{subfigure}
%DIF > 
  \begin{subfigure}{0.48\textwidth}
    \centering
    \includegraphics[width=1.0\textwidth]{figures/dinesh_2024_mixing_with_stirrer_2d/case_2/time0}
    \subcaption{\DIFaddFL{t = $7.38$ ms}}\label{fig:rings:ipst-nu-0.47-4}
  \end{subfigure}
  \caption{\DIFaddFL{Snapshots of particle positions with color indicating the stress
